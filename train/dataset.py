...
class CurriculumDataset(Dataset):
        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for line in f:
                if line.strip():
                    ...
                    if self.stage == "stage_a" and sample['mode'] != 'image_only':
                        continue
                    elif self.stage == "stage_b" and sample['mode'] != 'image_ehr':
                        continue
    ...
    def _log_statistics(self):
        ...
        stage_a_count = sum(1 for s in self.samples if s['mode'] == 'image_only')
        stage_b_count = sum(1 for s in self.samples if s['mode'] == 'image_ehr')
    ...
    def collate_fn(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.processor is not None:
            ...
            processed['labels'] = target_encoding['input_ids']
        else:
        ...
        # Add metadata
        processed['mode'] = [item['mode'] for item in batch]
        processed['stage'] = [item['stage'] for item in batch]
        processed['study_id'] = [item['study_id'] for item in batch]
