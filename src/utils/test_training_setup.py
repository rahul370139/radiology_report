...
def test_model_loading():
    """Test LLaVA-Med model loading."""
    logger.info("\n" + "=" * 70)
    logger.info("TEST 3: LLaVA-Med Model Loading")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Test processor loading for LLaVA-Med
        try:
            ...
            # Create image processor for LLaVA-Med (resize to 336x336)
            ...
            logger.error(f"❌ LLaVA-Med processor loading failed: {e}")
            return False
        
        # Test LLaVA-Med specific model loading
        try:
            from llava.model.language_model.llava_mistral import LlavaMistralForCausalLM
            logger.info("✅ LLaVA-Med model class imported successfully")
            logger.info("✅ LLaVA-Med model loading test passed")
            
        except ImportError as e:
            logger.warning(f"LLaVA-Med import failed: {e}")
            logger.info("Testing fallback to standard transformers...")
            
            try:
                from transformers import AutoModelForCausalLM
                logger.info("✅ Fallback model class available")
...
def test_imports():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("TEST 5: Required Imports")
    logger.info("=" * 70)
